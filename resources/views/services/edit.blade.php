    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <form action="{{ route('services.update', $service->id) }}" method="POST" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label for="icon" class="block text-sm font-medium text-gray-700 mb-1.5">Icône (HTML/SVG)</label>
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $service->icon) }}"
                                class="w-full rounded-lg border-gray-200 bg-gray-50 px-4 py-2.5 text-sm font-mono focus:border-blue-500 focus:ring-blue-500 transition-colors @error('icon') border-red-500 @enderror"
                                placeholder="<svg>...</svg> ou emoji">
                            <p class="mt-1.5 text-xs text-gray-400">Vous pouvez mettre un emoji 🚀 ou un SVG directement.</p>
                            @error('icon')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
