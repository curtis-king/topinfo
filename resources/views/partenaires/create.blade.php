    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Ajoutez un partenaire ou sponsor</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <form action="{{ route('partenaires.store') }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label for="logo" class="block text-sm font-medium text-gray-700 mb-1.5">Logo</label>
                            <div class="mt-1 flex items-center gap-4">
                                <label class="relative cursor-pointer bg-gray-50 hover:bg-gray-100 border-2 border-dashed border-gray-200 rounded-lg px-6 py-4 transition-colors">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-600">Cliquez pour ajouter un logo</p>
                                            <p class="text-xs text-gray-400">PNG, JPG, SVG (max. 2 Mo)</p>
                                        </div>
                                    </div>
                                    <input type="file" name="logo" id="logo" accept="image/*" class="hidden" onchange="document.getElementById('logo-preview').classList.remove('hidden'); document.getElementById('logo-preview').src = window.URL.createObjectURL(this.files[0])" required>
                                </label>
