    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">{{ $actuality->title }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <form action="{{ route('actuality.update', $actuality->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $actuality->title) }}"
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                ...
                                placeholder="Résumé ou accroche de l'article..." required>{{ old('description', $actuality->description) }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="10"
                                ...
                                placeholder="Contenu détaillé de l'article..." required>{{ old('content', $actuality->content) }}</textarea>
                            @error('content')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid sm:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="publication_date" id="publication_date" value="{{ old('publication_date', $actuality->publication_date) }}"
                            ...
                            <div>
                                ...
                                @if ($actuality->image_url)
                                    <div class="mb-3">
                                        <img src="{{ $actuality->image_url }}" alt="{{ $actuality->title }}" class="w-full h-24 rounded-lg object-cover border border-gray-200">
                                    </div>
                                @endif
                                <label for="image_path" class="block text-sm font-medium text-gray-700 mb-1.5">Nouvelle image (optionnelle)</label>
