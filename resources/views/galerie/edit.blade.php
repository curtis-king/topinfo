<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Modifier l'image</h2>
                <p class="text-sm text-gray-500">{{ $galerie->title }}</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <form action="{{ route('galerie.update', $galerie->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf @method('PUT')

                    <div class="space-y-6">
                        <div>
                            <label for="title" class="block text-sm font-medium text-gray-700 mb-1.5">Titre</label>
                            <input type="text" name="title" id="title" value="{{ old('title', $galerie->title) }}"
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3"
                                class="w-full rounded-lg border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-blue-500 focus:ring-blue-500 transition-colors @error('description') border-red-500 @enderror">{{ old('description', $galerie->description) }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if($galerie->image_url)
                                <img src="{{ $galerie->image_url }}" alt="{{ $galerie->title }}"
                                    class="w-full max-h-48 object-contain rounded-xl border border-gray-100 bg-gray-50 p-2 mb-3">
                            @endif

                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Remplacer l'image <span class="text-gray-400 font-normal">(optionnel)</span></label>
                            <div class="flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-300 transition-colors cursor-pointer" id="drop-zone">
                                <div class="space-y-2 text-center" id="drop-content">
                                    ...
                                    <div class="text-sm text-gray-500">
                                        <label for="cover_image" class="cursor-pointer text-blue-600 hover:text-blue-500 font-medium">Choisir une nouvelle image</label>
                                    </div>
                                    <p class="text-xs text-gray-400">PNG, JPG, WEBP — max 5 Mo</p>
                                </div>
                                <img id="preview" class="hidden max-h-40 rounded-lg mx-auto object-contain" alt="Aperçu">
                            </div>
