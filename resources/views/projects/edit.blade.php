    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="p-8">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                ...
                                placeholder="Brève description du projet..." required>{{ old('description', $project->description) }}</textarea>
                            @error('description')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="details" id="details" rows="6"
                                ...
                                placeholder="Description détaillée, technologies utilisées, défis relevés...">{{ old('details', $project->details) }}</textarea>
                            @error('details')
                                <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            @if ($project->image_url)
                                <div class="mb-3">
                                    <img src="{{ $project->image_url }}" alt="{{ $project->name }}" class="w-48 h-32 rounded-lg object-cover border border-gray-200">
                                </div>
                            @endif
                            <label for="image" class="block text-sm font-medium text-gray-700 mb-1.5">Nouvelle image (optionnelle)</label>
                            <div class="mt-1 flex items-center gap-4">
                                <label class="relative cursor-pointer bg-gray-50 hover:bg-gray-100 border-2 border-dashed border-gray-200 rounded-lg px-6 py-4 transition-colors">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="text-sm font-medium text-gray-600">Cliquez pour changer l'image</p>
